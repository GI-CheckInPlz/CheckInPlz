...
GameObject* CreateMonster2()
{
	...
	property->walkAnimation->PushImage(property->walkAnimation, L"Resources\\m2\\walk1.bmp");
	property->walkAnimation->PushImage(property->walkAnimation, L"Resources\\m2\\walk3.bmp");
	property->walkAnimation->PushImage(property->walkAnimation, L"Resources\\m2\\walk2.bmp");
	property->walkAnimation->PushImage(property->walkAnimation, L"Resources\\m2\\walk4.bmp");
	property->idleAnimation = CreateAnimation();
	property->idleAnimation->PushImage(property->idleAnimation, L"Resources\\m2\\idle1.bmp");
	property->idleAnimation->PushImage(property->idleAnimation, L"Resources\\m2\\idle2.bmp");
	property->jumpAnimation = CreateAnimation();
	property->jumpAnimation->PushImage(property->jumpAnimation, L"Resources\\m2\\jump1.bmp");
	property->jumpAnimation->PushImage(property->jumpAnimation, L"Resources\\m2\\jump2.bmp");
	property->fallAnimation = CreateAnimation();
	property->fallAnimation->PushImage(property->fallAnimation, L"Resources\\m2\\fall1.bmp");
	property->fallAnimation->PushImage(property->fallAnimation, L"Resources\\m2\\fall2.bmp");
	property->collideAnimation = CreateAnimation();
	property->collideAnimation->PushImage(property->collideAnimation, L"Resources\\m2\\collide.bmp");
	property->restAnimation = CreateAnimation();
	property->restAnimation->PushImage(property->restAnimation, L"Resources\\m2\\rest1.bmp");
	property->restAnimation->PushImage(property->restAnimation, L"Resources\\m2\\rest2.bmp");
	property->alpha = 1;
	property->type = 1;
	...
	obj->Draw = Monster2Draw;
	obj->Start = Monster2Start;
	obj->Update = Monster2Update;
	obj->OnCollisionEnter = Monster2Enter;
	obj->OnCollisionExit = Monster2Exit;
	obj->OnCollisionStay = Monster2Stay;
	obj->OnDestroy = Monster2OnDestory;
...
void Monster2Update(GameObject* this, double dt)
{
	...
	{		
		...
		{
			prop->fallCheck = true;
			timer.Delay(this, 0.1, false, Monster2StateTransition, Fall);
		}
...
void Monster2Enter(GameObject* this, CollisionResponse res)
{
	...
	{ // 나는 멈춰있는데
		if (CompareTag(res.other->tag, "Monster"))
		{
			...
			{ // 뒤에서 걸어서 나를 치면
				...
				{
					Monster2Collision(this, 1, 0);
					Monster2Collision(res.other, -1, 0);
				}
			}
			else if (yourProp->characterState == Fall)
			{ // 낙하 중에 나를 치면

				Monster2Collision(res.other, -1, -1);
	...
	{ // 내가 점프중인데
		...
		{ // 몬스터랑 충돌했고
			...
			{
				Monster2Collision(this, -1, -1);
				Monster2Collision(res.other, 1, 0); // 서있는 몬스터를 내가 점프중에 치는 경우는 왼쪽에서 점프해서 치는 경우만 존재
			}
			else if (yourProp->characterState == Jump)
			{
				//둘다 점프중일때
...
void Monster2Start(GameObject* this)
{
	...
	p->walkAnimation->Play(this, p->walkAnimation, 0.75, true);
...
void Monster2StandJump(GameObject* this)
{
	...
	timer.Delay(this, 0.5, false, Monster2StateTransition, Fall);
	prop->jumpAnimation->Play(this, prop->jumpAnimation, 0.3f, false);
	prop->characterState = Jump;
}
...
void Monster2DoubleJump(GameObject* this)
{
	...
	Animate(this, &this->transform.position.y, PLAYBACK_ONCE_FORWARD, this->transform.position.y + prop->doubleJumpFroce.y / 1.48f, EASING_OUTSINE, 0.6f, NULL);
	timer.Delay(this, 0.6, false, Monster2StateTransition, Fall);
