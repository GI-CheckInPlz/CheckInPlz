...
GameObject* CreateMonster3()
{
	...
	property->walkAnimation->PushImage(property->walkAnimation, L"Resources\\m3\\walk1.bmp");
	property->walkAnimation->PushImage(property->walkAnimation, L"Resources\\m3\\walk2.bmp");
	property->walkAnimation->PushImage(property->walkAnimation, L"Resources\\m3\\walk3.bmp");
	property->walkAnimation->PushImage(property->walkAnimation, L"Resources\\m3\\walk4.bmp");
	property->walkAnimation->PushImage(property->walkAnimation, L"Resources\\m3\\walk5.bmp");
	property->idleAnimation = CreateAnimation();
	property->idleAnimation->PushImage(property->idleAnimation, L"Resources\\m3\\idle1.bmp");
	property->idleAnimation->PushImage(property->idleAnimation, L"Resources\\m3\\idle2.bmp");
	property->jumpAnimation = CreateAnimation();
	property->jumpAnimation->PushImage(property->jumpAnimation, L"Resources\\m3\\jump1.bmp");
	property->jumpAnimation->PushImage(property->jumpAnimation, L"Resources\\m3\\jump2.bmp");
	property->fallAnimation = CreateAnimation();
	property->fallAnimation->PushImage(property->fallAnimation, L"Resources\\m3\\fall1.bmp");
	property->fallAnimation->PushImage(property->fallAnimation, L"Resources\\m3\\fall2.bmp");
	property->collideAnimation = CreateAnimation();
	property->collideAnimation->PushImage(property->collideAnimation, L"Resources\\m3\\collide.bmp");
	property->restAnimation = CreateAnimation();
	property->restAnimation->PushImage(property->restAnimation, L"Resources\\m3\\rest1.bmp");
	property->restAnimation->PushImage(property->restAnimation, L"Resources\\m3\\rest2.bmp");
	property->alpha = 1;
	property->type = 2;
	...
	obj->Draw = Monster3Draw;
	obj->Start = Monster3Start;
	obj->Update = Monster3Update;
	obj->OnCollisionEnter = Monster3Enter;
	obj->OnCollisionExit = Monster3Exit;
	obj->OnCollisionStay = Monster3Stay;
	obj->OnDestroy = Monster3OnDestory;
...
void Monster3Update(GameObject* this, double dt)
{
	...
	else if (prop->characterState == Walk)
	{
		...
		{
			prop->fallCheck = true;
			timer.Delay(this, 0.1, false, Monster3StateTransition, Fall);
		}
...
void Monster3Enter(GameObject* this, CollisionResponse res)
{
	...
	{ // 나는 멈춰있는데
		if (CompareTag(res.other->tag, "Monster")) 
		{
			...
			{ // 뒤에서 걸어서 나를 치면
				...
				{
					Monster3Collision(this, 1, 0);
					Monster3Collision(res.other, -1, 0);
				}
			}
			else if (yourProp->characterState == Fall)
			{ // 낙하 중에 나를 치면

				Monster3Collision(res.other, -1, -1);
	...
	{ // 내가 점프중인데
		...
		{ // 몬스터랑 충돌했고
			...
			{
				Monster3Collision(this, -1, -1);
				Monster3Collision(res.other, 1, 0); // 서있는 몬스터를 내가 점프중에 치는 경우는 왼쪽에서 점프해서 치는 경우만 존재
			}
			else if (yourProp->characterState == Jump)
			{
				//둘다 점프중일때
...
void Monster3Jump(GameObject* this, RaycastResponse res)
{
	...
	timer.Delay(this, 0.5, false, Monster3StateTransition, Fall);
...
void Monster3DoubleJump(GameObject* this)
{
	...
	timer.Delay(this, 0.6, false, Monster3StateTransition, Fall);
