...
GameObject* CreateMonster4()
{
	...
	property->walkAnimation->PushImage(property->walkAnimation, L"Resources\\m4\\walk1.bmp");
	property->walkAnimation->PushImage(property->walkAnimation, L"Resources\\m4\\walk2.bmp");
	property->walkAnimation->PushImage(property->walkAnimation, L"Resources\\m4\\walk3.bmp");
	property->walkAnimation->PushImage(property->walkAnimation, L"Resources\\m4\\walk4.bmp");
	property->idleAnimation = CreateAnimation();
	property->idleAnimation->PushImage(property->idleAnimation, L"Resources\\m4\\idle1.bmp");
	property->idleAnimation->PushImage(property->idleAnimation, L"Resources\\m4\\idle2.bmp");
	property->jumpAnimation = CreateAnimation();
	property->jumpAnimation->PushImage(property->jumpAnimation, L"Resources\\m4\\jump1.bmp");
	property->jumpAnimation->PushImage(property->jumpAnimation, L"Resources\\m4\\jump2.bmp");
	property->fallAnimation = CreateAnimation();
	property->fallAnimation->PushImage(property->fallAnimation, L"Resources\\m4\\fall1.bmp");
	property->fallAnimation->PushImage(property->fallAnimation, L"Resources\\m4\\fall2.bmp");
	property->collideAnimation = CreateAnimation();
	property->collideAnimation->PushImage(property->collideAnimation, L"Resources\\m4\\collide.bmp");
	property->restAnimation = CreateAnimation();
	property->restAnimation->PushImage(property->restAnimation, L"Resources\\m4\\rest1.bmp");
	property->restAnimation->PushImage(property->restAnimation, L"Resources\\m4\\rest2.bmp");
	property->alpha = 1;
	property->type = 3;
	...
	obj->Draw = Monster4Draw;
	obj->Start = Monster4Start;
	obj->Update = Monster4Update;
	obj->OnCollisionEnter = Monster4Enter;
	obj->OnCollisionExit = Monster4Exit;
	obj->OnCollisionStay = Monster4Stay;
	obj->OnDestroy = Monster4OnDestory;
...
void Monster4Update(GameObject* this, double dt)
{
	...
	else if (prop->characterState == Walk)
	{
		...
		{
			prop->fallCheck = true;
			timer.Delay(this, 0.1, false, Monster4StateTransition, Fall);
		}
...
void Monster4Enter(GameObject* this, CollisionResponse res)
{
	...
	{ // 나는 멈춰있는데
		if (CompareTag(res.other->tag, "Monster"))
		{
			...
			{ // 뒤에서 걸어서 나를 치면
				...
				{
					Monster4Collision(this, 1, 0);
					Monster4Collision(res.other, -1, 0);
				}
			}
			else if (yourProp->characterState == Fall)
			{ // 낙하 중에 나를 치면

				Monster4Collision(res.other, -1, -1);
	...
	{ // 내가 점프중인데
		...
		{ // 몬스터랑 충돌했고
			...
			{
				Monster4Collision(this, -1, -1);
				Monster4Collision(res.other, 1, 0); // 서있는 몬스터를 내가 점프중에 치는 경우는 왼쪽에서 점프해서 치는 경우만 존재
			}
			else if (yourProp->characterState == Jump)
			{
				//둘다 점프중일때
...
void Monster4Jump(GameObject* this, RaycastResponse res)
{
	...
	timer.Delay(this, 0.5, false, Monster4StateTransition, Fall);
...
void Monster4DoubleJump(GameObject* this)
{
	...
	timer.Delay(this, 0.6, false, Monster4StateTransition, Fall);
...
void Monster4Collision(GameObject* this, int xDir, int yDir)
{
	...
	AUDIO_PlayAudio("Resources\\Sound\\stump.wav", 2);
